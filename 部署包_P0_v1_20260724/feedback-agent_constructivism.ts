...
/** 从 history 提取用户最具代表性的发言 */
function extractUserQuote(history: SessionData['history']): string {
  if (!history || history.length === 0) return '';

  // 1. 优先取较长的发言（≥6字）
  for (let i = history.length - 1; i >= 0; i--) {
    if (history[i].role === 'user' && history[i].content.length >= 6) {
  ...
  for (let i = 0; i < history.length; i++) {
    if (history[i].role === 'user' && history[i].content.length > longest.length) {
      longest = history[i].content;
    }
  }
  return longest;
}
