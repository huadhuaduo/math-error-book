...
function classifyInput(text: string): string {
  ...
  if (/在忙|聊一下|在吗|有空|说.*话|谈.*话|打扰|方便|找你.*聊|跟你.*说|请教.*问题/.test(text) && text.length < 20) return 'greeting';
  return 'other';
}
...
export async function respond(
  ...
  // 克隆 state —— 不修改传入的引用（避免 React 同引用跳过重渲染）
  const state: EngineState = {
    round: inputState.round + 1,
    npcMood: inputState.npcMood,
    phase: inputState.phase,
    ctx: { ...inputState.ctx },
    scores: { ...inputState.scores },
    traps: { ...inputState.traps },
  };
